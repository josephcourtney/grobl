    ...
    def write(content: str) -> None:
        ...
        for attempt in range(1, MAX_CLIPBOARD_RETRIES + 1):
            try:
                ...
                logger.warning(
                    "clipboard copy timed out (attempt %s/%s)",
                    attempt,
                ...
                if attempt < MAX_CLIPBOARD_RETRIES:
                    logger.warning(
                        "clipboard copy failed; retrying (attempt %s/%s)",
                        attempt,
                        MAX_CLIPBOARD_RETRIES,
                    )
                    continue
        ...
        if last_error is not None:
            logger.error("clipboard copy failed after %s attempts", MAX_CLIPBOARD_RETRIES)
            raise last_error
