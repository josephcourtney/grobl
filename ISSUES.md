...
CLI responsibilities are currently scattered:

* `cli.scan`:

  * Performs some validation.
  * Handles certain `click.UsageError`s.
  * Handles `BrokenPipeError` when printing the summary.

* `cli.common._execute_with_handling`:

  * Maps core exceptions (`PathNotFoundError`, `ScanInterrupted`, etc.) to exit codes.

* `cli.root.main`:

  * Wraps `cli.main(...)` in its own `BrokenPipeError` handling.

Additionally:

* `ScanParams` (CLI-only) and `ScanOptions` (service-level) have very similar names, making their roles slightly opaque.
* `scan` uses `ctx = click.get_current_context()` inline only to raise two `UsageError`s.
* The effective table style is computed in `scan` (via `resolve_table_style`) and then also has a separate `summary_style` argument passed into `_execute_with_handling`, even though `ScanOptions` already has a `summary_style` field.

**Suggested approach**

* Centralize `BrokenPipeError` handling:

  * Introduce a helper (e.g. `exit_on_broken_pipe()`):
  ...
  * In both `main` and `scan`, replace duplicated try/except blocks with:
  ...
  * Consider going further: let only `main` catch `BrokenPipeError`, and allow subcommands to re-raise it.

* Standardize top-level CLI error handling:

  * Have a single “top-level error adapter” that wraps command invocation and maps known exceptions to exit codes.
  * Keep `cli.scan` focused on parsing and delegating.

* Clarify types and naming:

  * Rename `ScanParams` to something explicitly CLI-scoped (`CliScanParams` or `ScanCliParams`).

* Improve validation readability:

  * Factor validation out of `scan` into helpers:

    ```python
    def validate_payload_and_summary(params: CliScanParams, ctx: click.Context) -> None: ...
    def validate_sink_and_output(params: CliScanParams, ctx: click.Context) -> None: ...
    ```

* Clean up summary style wiring:

  * Compute the effective table style once in `scan`:

    ```python
    effective_style = resolve_table_style(params.summary_style)
    ```

  * Pass `effective_style` into `ScanOptions` (instead of the raw value).

  * Drop any separate `summary_style` parameter from `_execute_with_handling`; it should just use `options.summary_style`.

This reduces duplication, makes error handling easier to discover, and keeps CLI functions flatter and more readable.
