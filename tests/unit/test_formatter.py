...
def test_human_summary_compact_contains_totals() -> None:
    out = human_summary(["a"], 10, 20, 30, table="compact")
...
def test_human_summary_full_includes_title_and_tree() -> None:
    ...
    out = human_summary(lines, 2, 10, 12, table="full")
    assert " Project Summary " in out
    for ln in lines:
        assert ln in out
    assert "Total tokens: 12" in out
...
def test_human_summary_full_empty_tree_uses_title_width() -> None:
    ...
    out = human_summary([], 0, 0, 0, table="full")
    assert " Project Summary " in out
...
def test_human_summary_omits_disabled_totals() -> None:
    out = human_summary(
        ["file.txt"],
        10,
        20,
